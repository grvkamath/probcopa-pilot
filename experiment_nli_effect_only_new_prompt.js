...
function displayInstructionExample() {
    ...
    const contextText = example['asks-for'] === 'effect' ? 
        `<strong>Situation:</strong> ${example.premise}<br><strong>Possible Effect:</strong> ${example.hypothesis}` : 
        `<strong>Situation:</strong> ${example.premise}<br><strong>Possible Cause:</strong> ${example.hypothesis}`;
...
function showNegativeInstructionFeedback(example, likelihood, isLikelihoodAppropriate) {
    ...
    const contextText = datapoint['asks-for'] === 'effect' ? 
        `<strong>Situation:</strong> ${datapoint.premise}<br><strong>Possible Effect:</strong> ${datapoint.hypothesis}` : 
        `<strong>Situation:</strong> ${datapoint.premise}<br><strong>Possible Cause:</strong> ${datapoint.hypothesis}`;
    document.getElementById('choice-context').innerHTML = contextText;
